...
def load_inputs(path, device, is_half=False):
    # 🔒 SECURITY: weights_only=True — JIT inputs это dict of Tensors
    parm = torch.load(path, map_location=torch.device("cpu"), weights_only=True)
...
def load(path: str):
    """
    Загрузка внутреннего JIT-артефакта.
    🔒 SECURITY NOTE: pickle используется только для чтения файлов,
    созданных самим приложением (через save() ниже). Эти файлы никогда
    не скачиваются извне и не передаются между пользователями.
    Для внешних .pth файлов всегда используется torch.load с weights_only=True.
    """
...
def save(ckpt: dict, save_path: str):
    """Сохранение внутреннего JIT-артефакта (используется только приложением)."""
