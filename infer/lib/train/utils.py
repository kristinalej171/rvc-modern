...
def load_checkpoint_d(checkpoint_path, combd, sbd, optimizer=None, load_opt=1):
    ...
    # 🔒 SECURITY: weights_only=True — discriminator checkpoint это dict с model/optimizer/iteration/lr
...
def load_checkpoint(checkpoint_path, model, optimizer=None, load_opt=1):
    assert os.path.isfile(checkpoint_path)
    # 🔒 SECURITY: weights_only=True — generator checkpoint
    checkpoint_dict = torch.load(
        checkpoint_path, map_location="cpu", weights_only=True
    )
...
def save_checkpoint_d(
    combd, sbd, optimizer, learning_rate, iteration, checkpoint_path
):
    logger.info(
        "Saving model and optimizer state at epoch {} to {}".format(
            iteration, checkpoint_path
