    ...
    try:
        ...
        if hps.pretrainG != "":
            if rank == 0:
                ...
                # 🔒 SECURITY: weights_only=True — pretrained checkpoint
        ...
        if hps.pretrainD != "":
            if rank == 0:
                ...
                # 🔒 SECURITY: weights_only=True — pretrained discriminator
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(
                ...
                logger, [writer, writer_eval], cache,
            )
        else:
            train_and_evaluate(
                rank, epoch, hps, [net_g, net_d], [optim_g, optim_d],
    ...
    if hps.if_cache_data_in_gpu == True:
        ...
        if cache == []:
            for batch_idx, info in enumerate(train_loader):
                if hps.if_f0 == 1:
                    (phone, phone_lengths, pitch, pitchf, spec, spec_lengths,
                     wave, wave_lengths, sid) = info
                else:
                    (phone, phone_lengths, spec, spec_lengths,
                     wave, wave_lengths, sid) = info
                ...
                if hps.if_f0 == 1:
                    cache.append((batch_idx, (phone, phone_lengths, pitch, pitchf,
                                              spec, spec_lengths, wave, wave_lengths, sid)))
                else:
                    cache.append((batch_idx, (phone, phone_lengths, spec, spec_lengths,
                                              wave, wave_lengths, sid)))
        else:
            shuffle(cache)
    else:
